...
def test_sink_serializes_none_settings_values(tmp_path):
    ...
    sink.initialize_settings_table(flatten_component_settings("component", {"component_optional": None}), timestamp=1.0)
    sink._process(
        AxisArray(
            data=np.arange(6, dtype=float).reshape(3, 2),
            dims=["time", "ch"],
...
def test_sink_serializes_list_settings_values(tmp_path):
    ...
    sink.initialize_settings_table({"component_list": [1, 2]}, timestamp=1.0)
    sink._process(
        AxisArray(
            data=np.arange(6, dtype=float).reshape(3, 2),
            dims=["time", "ch"],
...
def test_sink_updates_list_settings_with_same_shape(tmp_path):
    ...
    sink.update_settings_table("component", {"component_list": [[5, 6], [7, 8]]}, timestamp=2.0)
    sink._process(
        AxisArray(
            data=np.arange(6, dtype=float).reshape(3, 2),
            dims=["time", "ch"],
...
def test_sink_rotates_file_on_incompatible_list_shape_update(tmp_path):
    ...
    sink._process(
        AxisArray(
            ...
            dims=["time", "ch"],
            axes={"time": AxisArray.TimeAxis(fs=100.0, offset=2.0)},
            key="sig",
        )
    )
    sink.close(write=False)
...
def test_sink_rotates_file_on_scalar_to_array_update(tmp_path):
    ...
    sink.initialize_settings_table({"component_value": "foo"}, timestamp=1.0)
    sink.update_settings_table("component", {"component_value": [1, 2, 3]}, timestamp=2.0)
    sink.close(write=False)
...
def test_sink_rotates_file_on_rank_change_update(tmp_path):
    ...
    sink.initialize_settings_table({"component_value": [1, 2, 3]}, timestamp=1.0)
    sink.update_settings_table("component", {"component_value": [[1, 2, 3]]}, timestamp=2.0)
    sink.close(write=False)
...
def test_sink_rotation_before_new_data_preserves_new_settings_file(tmp_path):
    ...
    sink.initialize_settings_table({"component_list": [1, 2]}, timestamp=1.0)
    sink.update_settings_table("component", {"component_list": [[1, 2], [3, 4]]}, timestamp=2.0)
    sink.close(write=False)
